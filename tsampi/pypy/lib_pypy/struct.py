...
def unpack_int(data,index,size,le):
    # START HACK
    # ord() seems unneeded... maybe I'm doing somethign wrong (TW)
    # Added this condition because sometimes it's a bytearray.
    # This seems like the smallest nonbreakign change.
    if type(data) is not bytearray:
        byte_data = bytearray(data)
        bytes = [(b) for b in byte_data[index:index+size]]
    else:
